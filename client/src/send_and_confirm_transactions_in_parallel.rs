use {
    ...
    tokio::{sync::RwLock, task::JoinHandle, time::{Instant, timeout}},
};
use anyhow::{anyhow};
...
// Define or modify this function to map anyhow::Error to TpuSenderError
fn map_anyhow_error(e: anyhow::Error) -> TpuSenderError {
    // You can add more sophisticated mapping logic here depending on the error details
    TpuSenderError::Custom(format!("External error: {}", e))
}
...
async fn sign_all_messages_and_send<T: Signers + ?Sized>(
    ...
    for (counter, (index, message)) in messages_with_index.iter().enumerate() {
        ...
        futures.push(async move {
            ...
            let result = timeout(
                Duration::from_secs(30), // 30 seconds timeout
                send_transaction_with_rpc_fallback(
                    rpc_client,
                    tpu_client,
                    transaction,
            ...
            match result {
                Ok(Ok(())) => Ok(()),  // Transaction sent successfully
                Ok(Err(e)) => Err(e),  // Error from send_transaction_with_rpc_fallback
                Err(_) => Err(map_anyhow_error(anyhow!("Transaction send timed out")))  // Timeout error
            }
        });
    }
